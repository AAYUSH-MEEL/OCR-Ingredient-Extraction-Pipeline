...
|   Raw product label photograph (JPG or PNG)                      |

|   Source: mobile phone camera                                    |

|   Saved to: data/raw_images/                                     |
...
|   - Library: OpenCV fastNlMeansDenoisingColored                  |

|   - Parameters: h=10, hColor=10, template=7x7, search=21x21     |

|   - Removes camera sensor grain                                  |
...
|   - Library: OpenCV createCLAHE in LAB colour space              |

|   - Parameters: clipLimit=3.0, tileGrid=8x8 (64 tiles)          |

|   - Equalises brightness in each tile independently              |

|   - Applied only to L channel (brightness, not colour)           |

|                                                                  |

|   Step C: Unsharp Masking                                        |

|   - Library: OpenCV GaussianBlur + addWeighted                   |

|   - Formula: Sharpened = 1.8 x Original - 0.8 x Blurred         |

|   - Enhances character stroke edges                              |

|                                                                  |

|   Output saved to: data/dl_preprocessed/                         |
...
|        STAGE 3 -- DEEP LEARNING PREPROCESSING (Real-ESRGAN)     |

|                                                                  |

|   Input: classically preprocessed image                          |

|   Model: RRDBNet (Residual in Residual Dense Block Network)      |

|   Weights: RealESRGAN_x4plus.pth (67MB, downloaded at runtime)  |

|                                                                  |

|   Process:                                                       |

|   1. Upscale image 4x using trained neural network               |

|   2. Network reconstructs fine character detail                  |

|   3. Resize back to original dimensions (Lanczos4)               |

|   4. Overwrite same file in dl_preprocessed/                     |

|                                                                  |

|   Output: overwritten in data/dl_preprocessed/                   |

|   Effect: small font text becomes sharper and more readable      |
...
|   | ResNet / PP-LCNet backbone                           |       |

|   | Extracts 4-level feature maps                        |       |
...
|   | Combines all scales into one feature map             |       |
...
|   | - Probability map P (where is text?)                 |       |

|   | - Threshold map T (how sharp is the boundary?)       |       |
...
|   | Find contours -> filter (score > 0.6)                |       |

|   | Unclip polygons outward by factor 1.5                |       |

|   | Output: bounding polygons around text regions        |       |

|   +------------------------------------------------------+       |

|                                                                  |

|   Sub-stage B: Direction Classifier (MobileNetV3)               |

|   - Each crop resized to 48 x 192 pixels                        |

|   - Class 0: upright, Class 1: upside-down (180 degrees)        |

|   - If Class 1 with confidence > 0.9: rotate crop               |

|                                                                  |

|   Sub-stage C: CRNN / SVTR + CTC Recognition                    |

|   - CNN extracts left-to-right feature sequence                  |

|   - BiLSTM (v1/v2) or SVTR Transformer (v3/v4) models context   |

|   - CTC decoder: argmax -> remove blanks -> merge repeats        |

|   - Output: text string + confidence score per crop              |

|                                                                  |

|   Final output: [(bounding_box, text_string, confidence), ...]  |
...
|       STAGE 5 -- INTERNVL3.5-1B MULTIMODAL POST-PROCESSING      |

|                                                                  |

|   Input 1: Preprocessed image (path from dl_preprocessed/)      |

|   Input 2: Raw OCR text string (joined from PaddleOCR output)   |

|                                                                  |

|   Vision branch:                                                 |

|   Image -> resize 448x448 -> split into 14x14 patches           |

|   -> InternViT encoder -> 256 visual tokens (1024-dim each)      |

|   -> MLP Projector (1024->4096) -> 256 tokens in LLM space      |

|                                                                  |

|   Text branch:                                                   |

|   OCR text + prompt -> Tokenizer -> N text tokens (4096-dim)     |

|                                                                  |

|   Fusion (concatenation into one sequence):                      |

|   [IMG_START][v1..v256][IMG_END][prompt tokens][OCR tokens]      |
...
|   Image = primary source of truth (set via prompt instruction)   |
...
|   Until: end-of-sequence token OR max_new_tokens reached         |

|                                                                  |

|   Output: corrected structured Markdown report per image         |
...
|   Sections: Product Summary, Directions, Ingredients,            |

|             Allergy Advice, Manufacturer Info                    |

|                                                                  |

|   Cell B: Ingredients-only bullet list per image                 |

|                                                                  |

|   CSV: outputs/OCR_results/ingredients_output.csv               |

|   Columns: Image Name | Extracted Ingredients                    |

|   Auto-downloaded to local machine when cell runs                |
...
### Vision Branch
Preprocessed image (PIL, RGB)

|

v

Resize to 448 x 448 pixels

|

v

Split into 14x14 pixel patches (1024 patches total)
...
Each patch -> vector representation

Outputs: 256 visual tokens (1024-dim each)
...
Layer 1: Linear 1024 -> 4096

GELU activation

Layer 2: Linear 4096 -> 4096

Outputs: 256 visual tokens now in LLM space (4096-dim)

### Text Branch
System prompt + raw OCR text

|

v

Tokenizer (word-piece splitting)

|

v

N text token vectors (4096-dim each)
